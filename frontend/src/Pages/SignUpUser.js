...
export default function SignUpUser() {
  ...
  const onSubmit = async (values) =>{
    ...
    const response = await axios.post('/registerUser', data).catch((err) =>{
      if(err && err.response)
        console.log(" Error: ",err);
        if(err.response.data.msg ==='Email allready exist plz try with another email'){
          toast.error('Email allready exist plz try with another email')
        }
    });
